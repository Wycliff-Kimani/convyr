...
async def get_business(user=Depends(get_current_user)):
    ...
    result = supabase.table("businesses").select(
        "id, name, email, phone_number, whatsapp_phone_number_id, whatsapp_business_account_id, subscription_plan, subscription_status, fallback_cooldown_minutes, created_at"
...
class BusinessSettingsInput(BaseModel):
    fallback_cooldown_minutes: int = Field(..., ge=1, description="Minimum minutes between fallback replies to the same contact")


@router.patch("/business/settings")
async def update_business_settings(data: BusinessSettingsInput, user=Depends(get_current_user)):
    business_id = user.get("business_id")

    try:
        supabase.table("businesses").update({
            "fallback_cooldown_minutes": data.fallback_cooldown_minutes,
        }).eq("id", business_id).execute()
    except Exception as e:
        logger.error(f"Failed to update settings for business {business_id}: {str(e)}")
